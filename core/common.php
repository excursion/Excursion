...
$url = parse_url($config['main_url']);
$system['scheme'] = strpos($_SERVER['SERVER_PROTOCOL'], 'HTTPS') === 0 ? 'https' : 'http';
$system['secure'] = $url['scheme'] == 'https' ? true : false;
$system['site_uri'] = $url['path'];
$system['domain'] = preg_replace('#^www\.#', '', $system['host']);
$system['host'] = $_SERVER['HTTP_HOST'];
if ($system['site_uri'][mb_strlen($system['site_uri']) - 1] != '/') $system['site_uri'] .= '/';
$system['port'] = empty($url['port']) || $_SERVER['SERVER_PORT'] == 80 ? '' : ':' . $url['port'];
$system['absolute_url'] = $system['scheme'] . '://' . $system['host'] . $system['port'] . $system['site_uri'];
...
$config['footer_js'] .= implode(",", $footer_js);
$config['header_js'] .= implode(",", $header_js);
$config['header_css'] .= implode(",", $header_css);
