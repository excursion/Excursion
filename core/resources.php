...
$R['input_checkbox'] = '<label><input type="checkbox" name="{$name}" id="{$name}" {$checked}{$attrs} /> {$title}</label>';
...
$R['input_textarea_editor'] =  '<textarea class="ckeditor" name="{$name}" id="editor" rows="{$rows}" cols="{$cols}"{$attrs}>{$value}</textarea>{$error}';
...
$R['forum_icon'] = '<img src="assets/images/forums/marker_{$value}_topic.gif" width="22" height="18" border="0" />';
$R['forums_quote'] = '<blockquote>{$postername}:<br />{$text}</blockquote>';
$R['forums_button'] = '<a class="dButton" href="{$url}">{$lang}</a>';

/**
 * Global
 */
 
$R['link'] = '<a href="{$url}">{$lang}</a>'; 
$R['code_title_page_num'] = ' (' . $lang['Page'] . ' {$num})';
$R['img'] = '<img src="{$url}" width="{$size}" />';
