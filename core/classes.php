	...
	function Register($un, $pwd, $pwd2, $email, $sq, $sq_answer)
	{
		...
		if($config['disablereg']=='no')
		{
			...
			if(!$excursion->error_found())
			{
				...
				$insert['theme'] = $config['default_theme'];
				$insert['lang'] = $config['default_language'];
				...
				{
					$this->sendValidationEmail($insert['email']);
...
class Excursion {
	...
	function create_title($mask, $params = array(), $escape = true)
	{
		global $config;
		$res = (!empty($config[$mask])) ? $config[$mask] : $mask;
	...
	function rc_link($url, $text, $attrs = '')
	{
		$link_attrs = $this->rc_attr_string($attrs);
	...
	function load_forumStructure()
	{
		...
		foreach ($sql->fetchAll() as $row)
		{
			...
			$structure['forums'][$row['code']] = array(
				'path' => $path[$row['path']],
				'tpath' => $tpath[$row['path']],
				'rpath' => $row['path'],
				'id' => $row['id'],
	...
	function generateTPL($base, $type = 'core', $admin = null)
	{
		global $user, $config;
		...
		if ($type == 'plugin')
		{
			$scan_dirs[] = "themes/{$user['theme']}/plugins/";
			$scan_dirs[] = "themes/{$user['theme']}/plugins/{$base[0]}/";
			$scan_dirs[] = "plugins/{$base[0]}/tpl/";
		}
		elseif ($type == 'admin')
		{
			$scan_dirs[] = "themes/admin/";
			$scan_dirs[] = "core/admin/tpl/";
		}
		else
		{
			$scan_dirs[] = "themes/{$user['theme']}/";
		}
	...
	function title($mask, $params = array(), $escape = true)
	{
		$res = $mask;
		is_array($params) ? $args = $params : mb_parse_str($params, $args);
		if (preg_match_all('#\{(.+?)\}#', $res, $matches, PREG_SET_ORDER))
	...
	function sendMail($fmail, $subject, $body, $headers='', $customtemplate = false, $additional_parameters = null, $html = false)
	{
		global $config;
		...
		{
			$sitemaintitle = mb_encode_mimeheader($config['title'], 'UTF-8', 'B', "\n");

			$headers = (empty($headers)) ? "From: \"" . $sitemaintitle . "\" <" . $config['admin_email'] . ">\n" . "Reply-To: <" . $config['admin_email'] . ">\n"
					: $headers;
			...
			if (!$customtemplate)
			{
				$body_params = array(
					...
					'SITE_DESCRIPTION' => $config['subtitle'],
				...
				$html = array('<p>','</p>','<div>','</div>','<span>','</span>','\n');
...
class RSS {
	...
	function parse_page_text($pag_id, $pag_type, $pag_text, $pag_pageurl, $pag_parser)
	{
	...
	function fix_pubdate($pubdate)
	{
		global $user;
		
		$tz = floatval($user['timezone']);
		$sign = $tz > 0 ? '+' : '-';
	...
	private function getDetails()
	{
		...
		$sql = $db->query("SELECT * FROM pages WHERE state > 0");
		while ($row = $sql->fetch())
		{
	...
	{
		global $db, $excursion, $system;
		
		$items = '';
		$sql = $db->query("SELECT * FROM pages WHERE state > 0 ORDER BY date DESC");
		while ($row = $sql->fetch())
		{
			$items .= '<item>
				<title>'. htmlspecialchars($row['title']) .'</title>
				<link><![CDATA['. $system['absolute_url'] . $excursion->url('page', 'c='.$row['cat'].'&id='.$row['id'], '', true) .']]></link>
				<description><![CDATA['. $this->parse_page_text($row['id'], '', $row['text'], $system['absolute_url'] . $excursion->url('page', 'c='.$row['cat'].'&id='.$row['id'], '', true), '') .']]></description>
				<pubDate>'. $this->fix_pubdate(date('r', $row['date'])) .'</pubDate>
			</item>';
		}
		$items .= '</channel>
				</rss>';
